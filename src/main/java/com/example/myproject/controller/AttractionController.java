...
public class AttractionController {
    ...
    @PostMapping("/write")
    public String attWrite(AttractionDTO attractionDTO, @RequestParam("attImgFile") List<MultipartFile> attImgFileList, Model model){
        if (attImgFileList.get(0).isEmpty() && attractionDTO.getAno() == null){
            model.addAttribute("errorMsg", "첫번째 상품 이미지는 팔수 입력 값입니다");
        }
        try {
    ...
    public void attRead(Long ano, Model model){
        ...
        try {
            ...
            model.addAttribute("att", new AttractionDTO());
        }
    }
